...
public class PlayerZone {
   ...
   */
  public static Cell[][] addPlayerZone(int rowStart, int colStart, int playerZoneHeight, int playerNum, Cell[][] cells, boolean asc) {
    int k = 0;
    if (asc) {
      for (int row = rowStart; row < rowStart + playerZoneHeight; row++) {
        for (int col = colStart - k; col <= colStart + k; col += 2) {
          ...
          Pawn pawn = new Pawn(playerNum,cells[row][col]);
        }
        k++;
      }
    }
